...
1) Variables:
    This section includes the main global variables used in the program:
        1.1) program_name: This variable stores the name of the program.
        1.2) income*: This variable stores the total income amount.
        1.3) Expense variables*:
            1.3.1) rent: this variable stores the total rent amount.
            1.3.2) utilities: this variable stores the total utilities amount.
            1.3.3) groceries: this variable stores the total groceries amount.
            1.3.4) other_expenses: this variable stores the total amount for other expenses.
        1.4) payment_id_num: this variable stores the payment ID number, which is added to the unique payment ID.
        1.5) menu**: this variable stores the menu options, which are:
            1.5.1) Option 1 - Add Transactions: allows users to add transactions.
                   Option 2 - Remove Transactions: allows users to remove transactions.
                   Option 3 - View/Filter Transactions: allows users to view or filter transactions.
                   Option 4 - View Remaining Balance: allows users to view the remaining balance.
                   Option 5 - View Total Income: allows users to view the total income.
                   Option 6 - View Total Expenses: allows users to view the total expenses.
                   Option 7 - View the Lowest Expense Category: allows users to view the lowest expense category.
                   Option 8 - View the Highest Expense Category: allows users to view the highest expense category.
                   Option 9 - Exit: allows the users to exit the program.
        1.6) transaction_category_options**: this variable stores the transaction category options, which are:
            1.6.1) Option 1 - Income: transactions related to income.
                   ...
                   Option 5 - Other Expenses: transactions related to any other expense besides the three main listed.

    * Please note that the income and expense variables are updated once users add or remove transactions.
    ** Please note that the menu and transaction_category_options were created in case they need to be reused in the future. For the moment, the transaction_category_options was used twice, but in case of scalability, the menu variable may also be reused.


2) Data Structures:
    This section includes the data structures used in the program:
        2.1) transactions: this is a dictionary used to store all the transactions inputted by the users.
        2.2) expenses_dict: this is a dictionary containing the expenses only, which is used to check the lowest and highest expense categories.


3) Functions:
    This section includes the functions used in the program:
        3.1) total_income(): this functions returns the total income, which is the income variable.
        3.2) total_expenses(): this functions returns the total expenses, which is the sum of the following variables: rent + utilities + groceries + other_expenses.
        3.3) remaining_balance(): this functions returns the remaining balance, which is the subtraction of the total_income() - the total_expenses() functions.
        3.4) lowest_expense_category(ref_dict): this functions returns the lowest expense category.
        3.5) highest_expense_category(ref_dict): this function returns the highest expense category.
        3.6) add_to_dict(dictionary, payment_id, category, day, month, amount): this is a core function that updates the dictionary with new transactions.
        3.7) filter_transactions(transactions, selected_category): this function allows users to filter transactions based on a specific category (e.g., "Rent").
"""
...
--- Section 2: Program Body ---
    ...
    4) Welcome Message:
        This section displays a welcome message to the user.
        
    5) Transaction Operation:
        This section prompts the user to input a transaction operation, which are:
            5.1) Option 1 - Add Transactions: allows users to add transactions.
                 Option 2 - Remove Transactions: allows users to remove transactions.
                 Option 3 - View/Filter Transactions: allows users to view or filter transactions.
                 Option 4 - View Remaining Balance: allows users to view the remaining balance.
                 Option 5 - View Total Income: allows users to view the total income.
    ...
    6) Add Transactions:
        This section allows the user to add transactions based on five different categories:
            6.1) Option 1 - Income: transactions related to income.
                 Option 2 - Rent: transactions related to rent.
                 Option 3 - Utilities: transactions related to utilities.
                 Option 4 - Groceries: transactions related to groceries.
                 Option 5 - Other Expenses: transactions related to any other expense besides the three main listed.
        
        Once users select the transaction category they want to add, the program ask for further details such as the day, month, and amount. After that, a unique payment ID is generated and the data is added to the transactions dictionary.
    
    7) Remove Transactions:
        This section allows users to remove transactions by prompting the user to input the transaction they want to remove. If the transaction exists, it is removed, otherwise, a message informing that the transaction was not found is displayed.
    
    8) View/Filter Transactions:
        This section allows the users to perform two operations: view all the transactions of filter transactions by category. If the former option is selected, all the transactions stored in the transactions dictionary are displayed. If the latter is chosen, the user will be asked which category they want to filter (e.g. "rent").                
    
    9) View Remaining Balance:
        This section displays the remaining balance, which is the total income subtracted by the total expenses.
    
    10) View Total Income:
        This section displays the total income.
    
    11) View Total Expenses:
        This section displays the total expenses.
    
    12) View the Lowest Expense Category:
        This section displays the lowest expense category and its amount.
    
    13) View the Highest Expense Category:
        This section displays the highest expense category and its amount.
