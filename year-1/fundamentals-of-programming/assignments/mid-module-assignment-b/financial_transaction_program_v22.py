...
This program is a console-based finance tracker. It lets users add and manage income and expenses, keep track of transactions, and view things like total income, total expenses, and remaining balance.
...
3) Functions:
    This section includes the functions used in the program:
        ...
        3.7) filter_transactions(transactions, selected_category): displays transactions filtered by a selected category (e.g.,e.g., "Rent").
...
# Stores expense totals separately to easily determine highest and lowest categories
...
def remaining_balance():
    # Calculates remaining balance by subtracting expenses from income
...
def add_to_dict(dictionary, payment_id, category, day, month, amount):
    ...
    # Formats the day to dd in case user inputs d instead of dd (e.g., 03 instead of 3)
    if len(day) == 1:
        day = f"0{day}"

    # Formats the month to mm in case user inputs m instead of mm (e.g., 03 instead of 3)
    if len(month) == 1:
        month = f"0{month}"
    ...
    # Stores transaction details using payment ID as the key
...
def filter_transactions(transactions, selected_category):
    ...
    # Iterates through all transactions and displays only those matching the selected category
...
while True:
    ...
    if menu_option_chosen == 1:
        ...
        while True:
            ...
            try:
                ...
                # Ensure category is within valid range (1–5)
                if transaction_category_chosen in [1, 2, 3, 4, 5]:
                    payment_id_num += 1

                    # Ensure valid day input (1–31)
                    while True:
                    ...
                    if transaction_category_chosen == 1:
                        ...
                        # Update rent total and keep expenses_dict consistent
                        rent += amount_chosen
                        ...
                        # Update utilities total and keep expenses_dict consistent
                        ...
                        # Update groceries total and keep expenses_dict consistent
                        ...
                        # Update other expenses total and keep expenses_dict consistent
    ...
    # --- 7. Remove Transactions --- #
        ...
        if len(transactions) == 0:
            ...
            # Convert input to uppercase to match stored transaction IDs
            ...
            if transaction_to_delete in transactions:
                ...
                # Adjust totals based on the category of the removed transaction
                if category == "Income":
                    income -= amount
    ...
    # --- 8. View/Filter Transactions --- #
        ...
        while True:
            ...
            if len(transactions) == 0:
                ...
                # Allows user to either view all transactions or filter by category
                if view_or_filter == 1:
                    print("Transactions:")
