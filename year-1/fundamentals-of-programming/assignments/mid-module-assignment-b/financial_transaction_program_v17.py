...
def add_to_dict(dictionary, category, day, month, amount, payment_id):
    """
    ...
    if menu_option_chosen == 1: # Add transactions
            ...
            try: # Validate transaction category chosen
                ...
                if transaction_category_chosen in [1, 2, 3, 4, 5]:
                    ...
                    if transaction_category_chosen == 1:
                        add_to_dict(transactions, "Income", day_chosen, month_chosen, amount_chosen, payment_id)
                        income += amount_chosen

                    elif transaction_category_chosen == 2:
                        add_to_dict(transactions, "Rent", day_chosen, month_chosen, amount_chosen, payment_id)
                        rent += amount_chosen
                        expenses_dict.update({"Rent": rent})

                    elif transaction_category_chosen == 3:
                        add_to_dict(transactions, "Utilities", day_chosen, month_chosen, amount_chosen, payment_id)
                        utilities += amount_chosen
                        expenses_dict.update({"Utilities": utilities})

                    elif transaction_category_chosen == 4:
                        add_to_dict(transactions, "Groceries", day_chosen, month_chosen, amount_chosen, payment_id)
                        groceries += amount_chosen
                        expenses_dict.update({"Groceries": groceries})

                    elif transaction_category_chosen == 5:
                        add_to_dict(transactions, "Other Expenses", day_chosen, month_chosen, amount_chosen, payment_id)
        ...
        if len(transactions) == 0:
            ...
            if transaction_to_delete in transactions:
                ...
                print(f"Transaction {transaction_to_delete} not found")
                print()

    elif menu_option_chosen == 3: # View/filter transactions
        while True:
            if len(transactions) == 0:
                print("No transactions")
                print()
